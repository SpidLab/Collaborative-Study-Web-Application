...
const CollaborationDetails = () => {
  ...
  // New State Variables for QC Functionality
  ...
  // New Function to handle QC Initiate
  const handleQcInitiate = async () => {
    setIsQcInitiateLoading(true);
    try {
      const response = await axios.post(
        `${URL}/api/dataset/${uuid}/qc`,
        {},
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  // New Function to handle QC Results
                    ...
                    secondary={
                      <Box sx={{ display: 'flex', gap: 2, mt: 1 }}>
                        <Tooltip
                          title={
                            !isQcInitiateEnabled
                              ? "You need to be a sender and all invited users have accepted invitations to initiate QC."
                              : "Click to initiate QC Calculations."
                          }
                          placement="bottom"
                        >
                          <span> {/* Span is necessary to handle disabled buttons */}
                            <Button
                              variant="contained"
                              color="primary"
                              onClick={handleQcInitiate}
                              disabled={!isQcInitiateEnabled || isQcInitiateLoading}
                              startIcon={isQcInitiateLoading && <CircularProgress size={20} color="inherit" />}
                            >
                              QC Initiate
                            </Button>
                          </span>
                        ...
                        <Tooltip
                          title={
                            !isQcResultsEnabled
                              ? "You need to initiate QC before fetching results."
                              : "Click to fetch QC Results."
                          }
                          placement="bottom"
                        >
                          <span>
                            <Button
                              ...
                              startIcon={isQcResultsLoading && <CircularProgress size={20} color="inherit" />}
                            >
                              QC Results
                            </Button>
                          </span>
